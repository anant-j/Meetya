...
module.exports = (postal_code="N6G 2J8", keyword="sushi", all_places_details=[], latlng_array=[], context, callback) => {
    callback(null, get_nearby_places(postal_code, keyword, all_places_details, latlng_array, context, callback));
}


function get_nearby_places(postal_code, keyword, all_places_details, latlng_array, context, callback) {
    callback(null, get_latlng_from_postcode(postal_code, latlng_array, keyword, all_places_details, context, callback));
}


function get_latlng_from_postcode(postal_code, latlng_array, keyword, all_places_details, context, callback) {
    getJSON(
        ...
        function(status, data) {
            ...
            callback(null, places_search(latlng_array, keyword, all_places_details, context, callback));
        }
    );
    // console.log(latlng_array[0]);
}

function places_search(latlng_array, keyword, all_places_details, context, callback) {
    console.log(latlng_array[0]);
    getJSON(
        ...
        function(status, data) {
            ...
            console.log("done");
            return callback(null, all_places_details);
        }
    );
}
